...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  string loc_loss_name;
  if (loc_loss_type_ == MultiBoxLossParameter_LocLossType_L2) {
    LayerParameter layer_param;
    loc_loss_name = this->layer_param_.name() + "_l2_loc";
    ...
    LayerParameter layer_param;
    loc_loss_name = this->layer_param_.name() + "_smooth_L1_loc";
  ...
  for(int top_id=0;top_id<loc_bottom_vec_.size();top_id++)
  {
    LOG_IF(INFO, Caffe::root_solver())
      <<loc_loss_name<< " bottom shape: " << loc_bottom_vec_[top_id]->shape_string();
  }
  ...
  if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
    ...
    CHECK_LT(background_label_id_, num_classes_)
        << "background_label_id should be within [0, num_classes) for Softmax.";
    LayerParameter layer_param;
    conf_loss_name = this->layer_param_.name() + "_softmax_conf";
    ...
    LayerParameter layer_param;
    conf_loss_name = this->layer_param_.name() + "_logistic_conf";
  ...
  for(int top_id=0;top_id<conf_bottom_vec_.size();top_id++)
  {
    LOG_IF(INFO, Caffe::root_solver())
      <<conf_loss_name<< "bottom shape: " << conf_bottom_vec_[top_id]->shape_string();
  }
}
