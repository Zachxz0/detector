...
void Server::setAddress(const string& addr)
{
	this->m_addr = addr;
}
...
void Server::sendMessage(const char* msg,int size)
{
	...
	for (int i = 0; i <= m_max_fd; ++i)
	{
		...
		{
			int len = send(i,msg,size,0);
			//printf("%s %d %s %d\n","send len::",len, "required to send size :",size);
		}
	}
}

void Server::sendMessage(int fd,const char* msg,int size)
{
	if(m_listenfd == fd)return;
	m_current_wdfs = m_global_wdfs;
	if(FD_ISSET(fd,&m_current_wdfs))
	{
		int len = send(fd,msg,size,0);
	}
}
		...
		for(int i = 0; i <= m_max_fd; i++)
		{
			...
			if(FD_ISSET(i, &m_current_rdfs))
			{
				if(m_listenfd == i){
					...
					m_callback->hasClientConn(m_sfd);
					...
					if(m_callback!=NULL)
					{
						m_callback->getClientMessage(i,m_buff,bytes);
					}
				}
			}
		}
